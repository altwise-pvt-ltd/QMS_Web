...
const InstrumentDetailModal = ({ item, onClose }) => {
  ...
  const handleFileUpdate = (label, file) => {
    if (file) {
      ...
      alert(
        `File "${file.name}" selected for ${label}. Update functionality implemented in UI.`,
      );
    }
  };
  ...
  const DocItem = ({ label, value }) => (
    <div
      ...
      className={`w-full flex items-center justify-between p-10 mt-2 rounded-4xl border transition-all group relative overflow-hidden snap-center ${
        value !== "N/A"
    ...
    >
      <div className="flex items-center gap-5 min-w-0 flex-1">
        <div
          className={`w-14 h-14 rounded-2xl shrink-0 flex items-center justify-center shadow-inner transition-colors ${
            value !== "N/A"
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[10px] font-black uppercase text-indigo-600/60 tracking-widest leading-none mb-1.5">
            {label}
          </p>
          <p
            className={`text-[15px] font-black truncate transition-colors ${
              value !== "N/A"
      ...
      >
        <label
          className={`px-6 py-3.5 border rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer shadow-sm ${
            value !== "N/A"
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="p-5 bg-slate-50 rounded-3xl border border-slate-100 space-y-1">
                  ...
                  <p
                    className={`font-bold ${isExpired ? "text-rose-600" : "text-slate-700"}`}
                  >
                    {item.expiryDate || "N/A"}
                  </p>
                </div>
                <div className="p-5 bg-slate-50 rounded-3xl border border-slate-100 space-y-1">
                  ...
                  <p
                    className={`font-bold ${isExpired ? "text-rose-600" : "text-emerald-600"}`}
                  >
                    {isExpired ? "Needs Calibration" : "Operational"}
                  </p>
                </div>
              </div>
            </div>
