...
const InstrumentDetailModal = ({ item, onClose }) => {
  ...
  const handleFileUpdate = async (label, file) => {
    if (file) {
      try {
        ...
        const payload = {
          instrumentNomenclature: item.name,
          operatingDepartment: item.department,
          preventiveMaintenanceNotes:
            item.maintenanceText || "Updated via Profile View",
          expiryDate: item.expiryDate,
          status: "Active",
          // Set only the updated file, the service will preserve the rest if we leave them out
          // Actually, we should pass the current URL strings for the rest to be safe
          purchaseOrderFile:
            fieldName === "purchaseOrderFile" ? file : item.purchaseOrder,
          billReceiptFile:
            fieldName === "billReceiptFile" ? file : item.billReceipt,
          installationReportFile:
            fieldName === "installationReportFile"
              ? file
              : item.installationReport,
          iqOqPqProtocolFile:
            fieldName === "iqOqPqProtocolFile" ? file : item.iqOqPq,
          userOperationsManualFile:
            fieldName === "userOperationsManualFile" ? file : item.userManual,
          equipmentPhotographFile: item.photo,
          latestCalibrationCert:
            fieldName === "latestCalibrationCert" ? file : item.calibrationCert,
        };

        await instrumentService.updateInstrument(
          item.instrumentCalibrationId,
          payload,
        );
  ...
  const DocItem = ({ label, value, typeLabel = "PDF" }) => (
    <div
      className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 border rounded-xl bg-white group transition-all gap-4 ${value && value !== "N/A" ? "border-slate-200 hover:border-emerald-200 shadow-sm hover:shadow-md" : "border-slate-100 opacity-60"}`}
    >
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b bg-white">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 font-medium italic">
              Detailed equipment profile and documentation archive
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              <div className="flex items-center justify-center p-1.5 border border-slate-100 rounded-2xl bg-white shadow-sm self-center">
                {item.photo ? (
                  <img
                    src={item.photo}
                    alt={item.name}
                    className="w-full h-50 object-cover rounded-xl border border-slate-50"
                  />
                ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div
                    className={`px-4 py-3 border rounded-xl font-bold flex items-center justify-between ${isExpired ? "bg-rose-50 border-rose-100 text-rose-600" : "bg-emerald-50 border-emerald-100 text-emerald-600"}`}
                  >
              ...
              <div className="space-y-1.5">
                ...
                <div className="px-5 py-4 bg-slate-50 border border-slate-100 rounded-xl text-sm text-slate-600 min-h-[120px] italic leading-relaxed">
                  {item.maintenanceText ||
                    "No maintenance history recorded for this unit."}
                </div>
              </div>
            </div>
          </div>
    ...
    <div className="w-full space-y-4">
      ...
      {instruments.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className="group bg-white rounded-[32px] border border-slate-100 hover:border-emerald-200 shadow-sm hover:shadow-xl hover:shadow-emerald-500/5 transition-all duration-300 overflow-hidden px-6 sm:px-10 py-6 flex flex-col md:grid md:grid-cols-12 items-center gap-6 md:gap-4"
          >
            <div className="col-span-5 flex items-center gap-3 sm:gap-6 w-full md:w-auto">
              {item.photo ? (
                <img
                  src={item.photo}
                  alt={item.name}
                  className="w-16 h-16 object-cover rounded-2xl shadow-sm border border-slate-50 shrink-0"
                />
              ) : (
              ...
              <div className="min-w-0">
                <h4 className="text-lg font-black text-slate-900 tracking-tight group-hover:text-emerald-600 transition-colors uppercase italic truncate">
                  {item.name}
                </h4>
