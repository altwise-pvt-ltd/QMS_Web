...
const StatsCard = ({ title, sublabel, value, icon: Icon, accent }) => (
  <div
      ...
      if (apiData && Array.isArray(apiData) && apiData.length > 0) {
        ...
        const formattedCategories = apiData.map((cat, index) => {
          ...
          if (typeof cat === "string") {
            ...
            itemName =
              cat.riskCategoryName || cat.categoryName || cat.name || "Unknown";
  ...
  const handleSaveIndicator = async () => {
    ...
    try {
      if (editingIndicator) {
        ...
        const apiId =
          editingIndicator.riskIndicatorId ||
          (typeof editingIndicator.id === "number" ? editingIndicator.id : 0);
  ...
  const IndicatorCard = ({ indicator }) => {
    ...
    return (
      <div
        ...
        className={`bg-white group rounded-2xl border-2 transition-all p-5 flex flex-col justify-between h-full hover:-translate-y-1 shadow-sm hover:shadow-xl cursor-pointer ${
          isOverThreshold ? "border-rose-200" : "border-slate-100"
        }`}
      >
        <div>
          <div className="flex justify-between items-start mb-4">
            <span
              className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-indigo-600 text-white`}
            >
              {indicator.category}
            </span>
            ...
            <div className="flex items-center gap-2">
              ...
              {isOverThreshold ? (
                ...
                <TrendingUp
                  size={16}
                  className={
                    indicator.count > 10 ? "text-rose-500" : "text-emerald-500"
                  }
                />
              )}
            </div>
