  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        array.forEach((item, index) => {
          Object.keys(item).forEach((key) => {
            if (item[key]) {
              ...
              if (key === fileField) {
                if (item[key]) {
                  data.append(`${prefix}[${index}].${backendKey}`, item[key]);
                }
              } else {
                data.append(`${prefix}[${index}].${backendKey}`, item[key]);
              }
      ...
      formData.trainingRecords.forEach((item, index) => {
        if (item.title) {
          data.append(`Trainings[${index}].TrainingTitle`, item.title);
        }
        if (item.inductionTraining) {
          data.append(
            `Trainings[${index}].InductionFile`,
            item.inductionTraining,
          );
        }
        if (item.competencyTraining) {
          data.append(
            `Trainings[${index}].CompetencyFile`,
