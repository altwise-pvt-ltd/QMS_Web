      ...
      <div className={`space-y-6 ${standalone ? 'max-w-4xl mx-auto' : ''}`}>
        ...
        <div className={`bg-white/80 backdrop-blur-md p-4 rounded-2xl border border-slate-200 shadow-2xl flex items-center justify-between gap-4 ${standalone ? 'sticky bottom-6' : 'mt-8'}`}>
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="flex-1 sm:flex-none px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-200"
          >
            {isSaving ? (
