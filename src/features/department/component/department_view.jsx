...
export const DepartmentView = ({ deptId, deptName }) => {
  ...
  useEffect(() => {
    const fetchDepartmentStaff = async () => {
      ...
      try {
        ...
        const fullList = Array.isArray(responseData)
          ? responseData
          : responseData?.value || [];

        // Find the specific department we need using the ID
        const targetDept = fullList.find(
        ...
        if (targetDept && targetDept.staffList) {
          ...
          const nameMatch = fullList.find(
            (d) => d.departmentName?.trim().toLowerCase() === cleanName,
          );
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-100 animate-in slide-in-from-top-4 duration-300">
      ...
      {error && (
        <p className="text-xs text-red-500 text-center py-4 font-medium">
          {error}
        </p>
      )}
