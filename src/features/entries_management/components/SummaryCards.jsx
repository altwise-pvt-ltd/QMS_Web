  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {cards.map((card) => {
        ...
        const share =
          counts.all > 0 ? Math.round((card.count / counts.all) * 100) : 0;

        return (
          <button
            key={card.key}
            onClick={() => onFilter(card.key)}
            className={`relative rounded-xl overflow-hidden text-left transition-all active:scale-95 border border-slate-200 bg-white ${
              active
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-md border border-slate-100 ${card.bg} ${card.text}`}
                >
                  {card.trend}
                </div>
              </div>
              ...
              <div className="mb-4">
                <h3
                  className={`text-3xl font-bold leading-none tracking-tight ${active ? "text-slate-900" : card.text}`}
                >
                  {card.count}
                </h3>
